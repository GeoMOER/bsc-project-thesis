...
The following examples illustrates the reading of DWD RADOLAN files and the 
extraction of measurement time series at specific points defined by a point 
vector dataset. For simplicity, we start with reading flat binary RST files.
...
3. Extract points (or compute any other GIS operation)
...
lahnberge <- readOGR("vector/lahnberge.shp", "lahnberge")
...
### Do some analysis
With the above example, we have read the complete RADOLAN RW information for 2010 for the four locations, i.e. we have hourly values of rainfall amount at each of the four locations for every hour in 2010.
...
Subset the dataset to raining hours only (i.e. at least one of the four points
